...
define('text!src/common/sidebar/live/templates/course-direct.hbs',[],function () { return '<h2>LA COURSE EN DIRECT</h2>\n<a href="" class="audio-button live-button" data-type="audio"><span> </span></a>\n<a href="" class="video-button live-button" data-type="video"><span> </span></a>\n';});

define('src/common/equidia_player/EquidiaPlayerModel',['defaults'], function (Defaults) {
    

    return Backbone.Model.extend({
        setUrlAndFetch: function (mediaType) {
            if (mediaType === 'audio') {
                this.url = Defaults.LIVE_AUDIO;
            } else {
                this.url = Defaults.LIVE_VIDEO;
            }
            this.fetch();
        }
    });
});
define('src/common/equidia_player/EquidiaPlayerView',[
    'conf',
    './EquidiaPlayerModel'
], function (conf, EquidiaPlayerModel) {
    

    var EquidiaPlayerView = Backbone.View.extend({

        id: 'equidia-player',

        initialize: function () {
            this.model = new EquidiaPlayerModel();
            this.listenTo(this.model, 'sync', this.renderLive);
        ...
        show: function (type) {
            if (type === 'audio') {
                this.playerConfig = {width: 550, height: 100};
            } else {
                this.playerConfig = {width: 480, height: 272};
            }
            this.model.setUrlAndFetch(type);
            return this;
        },

        renderLive: function (model) {
            this.openMedia(model.get('url'), this.playerConfig);
        },

        openMedia: function (url, options) {
            options = options || {};
            window.open(url, options.titre || "", "width=" + options.width + ",height=" + options.height + ",resizable=yes,scrollbars=no");
        },

        renderError: function () {
            this.openMedia('', {titre: 'Erreur - pas de course en cours', width: 550, height: 100});
        }
    });

    return new EquidiaPlayerView();
});
...
define('src/common/sidebar/live/CourseDirectView',[
    'template!./templates/course-direct.hbs',
    'src/common/equidia_player/EquidiaPlayerView',
    'tagCommander'
], function (template, equidiaPlayerView, tagCommander) {
    ...
    return Backbone.View.extend({
        ...
        className: 'course-direct bloc clearfix',

        events: {
            'click .live-button': 'showPlayer'
        },
        ...
        showPlayer: function (event) {
            ...
            targetType = $(event.currentTarget).data().type;

            this.sendTag(targetType);

            equidiaPlayerView.show(targetType);
        },

        sendTag: function (type) {
            tagCommander.sendTags(
                'commun.clic.bandeau_droite.course_en_direct.' + type,
                'commun.clic.bloc_droit.course_en_direct.' + type
            );
        }
    });
});

define('CourseDirectStandalone',['require','core','timeManager','src/common/sidebar/live/CourseDirectView'],function (require) {
    

    require('core');
    var timeManager = require('timeManager'),
        CourseDirectView = require('src/common/sidebar/live/CourseDirectView');

    timeManager.fetch({
        success: function () {
            $('#bloc-course-direct').append(new CourseDirectView().render().el);
        },
        error: function (model) {
            model.error("CourseDirectView, erreur fetch timestamp");
        }
    });
});
