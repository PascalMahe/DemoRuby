...
class JSONCrawler
  ...
  def fetch_meetings(date, current_job)
		...
		@logger.info("fetch_meetings - Getting JSON meeting @: " + url_as_str)
	...
	def fetch_meeting_shallow(date, number, job, jsonMeeting)
		...
		@logger.debug("Country: '" + country.to_s + "'")
		...
		# track_condition
		track_condition = nil
		penetrometre_json = jsonMeeting["courses"][0]["penetrometre"]
		if penetrometre_json != nil then
			track_condition_raw = penetrometre_json["valeurMesure"]
	...
	def fetch_race(jsonRace, meeting_nb, date)
		...
		#Fetching page
		@logger.info("fetch_race - Fetching race: " + race_nb.to_s)
	...
	def fetch_race_shallow(jsonRace, race_nb)
		...
		# value
		value = jsonRace["montantPrix"].to_i
		...
		# update: 2018-02-08, bets are not in JSON, commenting it for now
		# bets = ""
		...
		rope = @ref_list_hash[:ref_rope_list][rope_raw]

		sex_rule_raw = jsonRace["conditionSexe"]
		sex_rule = @ref_list_hash[:ref_sex_rule_list][sex_rule_raw]

		general_cond_str = rope.to_s + " " + sex_rule.to_s
		...
		if jsonRace["arriveeDefinitive"] == true then
			jsonResult = jsonRace["ordreArrivee"]
			result = ""
			jsonResult.each do |current_index|
				if result.size() > 0 then
					result = result + " - "
				end
				result = result + current_index[0].to_s
			end
		...
		# time
		timestamp = jsonRace["heureDepart"].to_i
		timestamp = timestamp / 1000 # because it's in nanoseconds?!
		time = Time.at(timestamp)
		@logger.debug("fetch_race_shallow - time (local): " +
			time.strftime(@config[:gen][:default_date_time_format])
		)
